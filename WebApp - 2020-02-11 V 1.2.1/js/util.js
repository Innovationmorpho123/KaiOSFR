...
/* This function creates License File on SD Card.
contentToWrite: is the text to be written to file*/
function createLicenseFileOnSDCard(contentToWrite) {
    var sdcard = navigator.getDeviceStorage('sdcard');
    var request = sdcard.getEditable(CONST_LICENSE_FILE_NAME);
    request.onsuccess = function() {
        var file = this.result;
        console.log("File Exists, Reading the File Now " + file.name);
        writeLicenseDataToFile(contentToWrite);
    }
    request.onerror = function(error) {
        if (error.name = 'NotFoundError') {
            console.warn("NotFoundError");
...
/* This function writes License Data to SD Card.
contentToWrite: is the text to be written to file*/
function writeLicenseDataToFile(contentToWrite) {
    var sdcard = navigator.getDeviceStorages("sdcard");
    var file = new Blob([contentToWrite], { type: "text/plain" });
    var request = sdcard[0].addNamed(file, CONST_LICENSE_FILE_NAME);

    request.onsuccess = function() {
        var name = this.result;
        console.log("License File" + name + '" successfully wrote on the sdcard storage area');
    }
    request.onerror = function(error) {
        console.warn("Unable to write License file: " + JSON.stringify(error));
    }
}
//Access File System